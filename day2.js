let gameConfig = { red: 12, green: 13, blue: 14 };

let dataSample = ["Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green",
    "Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue",
    "Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red",
    "Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red",
    "Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green"]

let data = ["Game 1: 5 red, 1 green; 6 red, 3 blue; 9 red; 1 blue, 1 green, 4 red; 1 green, 2 blue; 2 blue, 1 red",
...
let games = data.map(game => {
    let splitGameData = game.split(':');

    let gameObject = {
        gameID: parseInt(splitGameData[0].replace('Game ', '')), sets: [], vaildGame: true, power: 0
    }
    let gameSets = splitGameData[1].split(';');
    let gameMinimumCubes = { red: 0, green: 0, blue: 0 };
    gameObject.sets = gameSets.map(set => {
        let cubes = set.split(',');
        let detail = { red: 0, green: 0, blue: 0 }
        cubes.forEach(cube => {
            let cubeDetails = cube.trimStart().split(' ');
            let cubeCount = parseInt(cubeDetails[0]);
            detail[cubeDetails[1]] = cubeCount;
            if (gameConfig[cubeDetails[1]] < cubeDetails[0]) {
                gameObject.vaildGame = false;
            }

            if (gameMinimumCubes[cubeDetails[1]] < cubeCount) {
                gameMinimumCubes[cubeDetails[1]] = cubeCount;
            }
    ...
    gameObject.power = gameMinimumCubes.red * gameMinimumCubes.green * gameMinimumCubes.blue;
...
let vaildGames = games.filter(game => game.vaildGame);

let starOne = vaildGames.reduce((sum, game) => sum += game.gameID, 0);
console.log(`starOne: ${starOne}`);

let starTwo = games.reduce((sum, game) => sum += game.power, 0);
