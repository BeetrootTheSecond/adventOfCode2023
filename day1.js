...
//remove all letters for string 

let dataNumbers = data.map(row => [...row].filter(char => !isNaN(char))
)


//console.log(dataNumbers.toString());

let firsLastNumbers = dataNumbers.map(no => {

    return parseInt(`${no[0]}${no[no.length - 1]}`);
})

//console.log(firsLastNumbers);

let starOne = firsLastNumbers.reduce((count, row) => count + row, 0);

console.log(`star one ${starOne}`);



let dataCorrected = data.map(row => {


    row = row.replace('one', '1one');
    row = row.replace('two', '2two');
    row = row.replace('three', '3three');
    row = row.replace('four', '4four');
    row = row.replace('five', '5five');
    row = row.replace('six', '6six');
    row = row.replace('seven', '7seven');
    row = row.replace('eight', '8eight');
    row = row.replace('nine', '9nine');

    return row;
})


let dataNumbersTwo = dataCorrected.map(row => [...row].filter(char => !isNaN(char))
)

let secondtry = data.map(row => {
    ...
    for (let index = 0; index < rowArray.length; index++) {
        ...
        let sliced = row.slice(index);

        if (sliced.slice(0, 3) == 'one') {
            numberArray.push(1);
            continue;
        }
        if (sliced.slice(0, 3) == 'two') {
            numberArray.push(2);
            continue;
        }
        if (sliced.slice(0, 5) == 'three') {
            numberArray.push(3);
            continue;
        }
        if (sliced.slice(0, 4) == 'four') {
            numberArray.push(4);
            continue;
        }
        if (sliced.slice(0, 4) == 'five') {
            numberArray.push(5);
            continue;
        }
        if (sliced.slice(0, 3) == 'six') {
            numberArray.push(6);
            continue;
        }
        if (sliced.slice(0, 5) == 'seven') {
            numberArray.push(7);
            continue;
        }
        if (sliced.slice(0, 5) == 'eight') {
            numberArray.push(8);
            continue;
        }
        if (sliced.slice(0, 4) == 'nine') {
            numberArray.push(9);
            continue;
        }
...
let firsLastNumbersTwo = secondtry.map(no => {
...
let starTwo = firsLastNumbersTwo.reduce((count, row) => count + row, 0);
